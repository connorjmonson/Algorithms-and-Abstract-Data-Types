...
public class List {
 ...
 boolean equals(List L) { //Using code provided by professor in examples (ended up doing that a decent amount) 
  boolean flag  = true;
  Node N = this.front;
  Node M = L.front;
 ...
 //Pre: length()>0, index()>=0
 void insertAfter(int data) { //basically same as insertBefore
  if ( !(length()>0) ) {
 ...
 List copy(){
  List newListCopy = new List();
  Node nodeCopy = this.front;
  while(nodeCopy != null) {
   newListCopy.append(nodeCopy.data);//Append node to new list
